...
export class AppComponent implements OnInit {
  ...
  private resolveUidAndToken(): { uid: string | null; token: string | null } {
    ...
    const uidFromHref = uidFromHrefMatch
      ? decodeURIComponent(uidFromHrefMatch[1])
      : null;
    ...
    this.apiService
      ...
      .pipe(
        tap((response: any) => {
          ...
          const apiLang = (response?.flight?.flight_search?.language || '')
            .toString()
            .toLowerCase();
      ...
      .subscribe((pdpaResponse: any) => {
        ...
        if (pdpaResponse.consent) {
          ...
          const isPaymentRedirect = currentPath === '/payment-page';
